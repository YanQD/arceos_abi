...
use axconfig::KERNEL_PROCESS_ID;
...
use crate::{
    current_processor, processor::Processor, schedule::add_wait_for_exit_queue, taskctx::{current_task_ptr, set_current_task_ptr, TaskInner}, AxTask, AxTaskRef
};
...
impl CurrentTask {
    pub(crate) fn try_get() -> Option<Self> {
        let ptr: *const super::AxTask = current_task_ptr();
        if !ptr.is_null() {
    ...
    pub(crate) unsafe fn init_current(init_task: AxTaskRef) {
        ...
        unsafe { set_current_task_ptr(ptr) };
    }
...
extern "C" fn task_entry() -> ! {
    ...
    if let Some(entry) = task.get_entry() {
        if task.get_process_id() == KERNEL_PROCESS_ID {
            ...
            // 切换页表已经在switch实现了
            // 记得更新时间
            task.time_stat_from_kernel_to_user(axhal::time::monotonic_time_nanos() as usize);
